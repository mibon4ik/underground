    ...
    if (this.state.error) {
      return (
        <div style={{
          ...
          flexDirection: "column", background: "#03060B", color: "#D8FF47", padding: 40, textAlign: "center"
        }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>⚠️</div>
          <h2 style={{ fontSize: 22, fontWeight: 1000, marginBottom: 12, color: "#eef4fb" }}>
            Произошла ошибка
          </h2>
          <p style={{ color: "#97a3b7", maxWidth: 400, lineHeight: 1.7, marginBottom: 20, fontSize: 14 }}>
            Не удалось загрузить страницу. Убедитесь что бекенд запущен:
          </p>
          <code style={{
            background: "#0D121F", padding: "10px 16px", borderRadius: 10,
            border: "1px solid #273146", color: "#D8FF47", fontSize: 13, marginBottom: 20
          }}>
          ...
          <button onClick={() => this.setState({ error: null })}
            style={{
              ...
              background: "#D8FF47", color: "#0a0d12", fontSize: 14,
...
const STATIC_SEARCH = {
  ...
  contacts: "контакты адрес телефон филиал связь обратная связь",
};
...
export default function App() {
  ...
  useEffect(() => {
    let b = import.meta.env.VITE_API_URL || '/api'; if (!b.startsWith('http') && !b.startsWith('/')) b = 'https://' + b; fetch(`${b.startsWith('http') ? b.replace(/\/?$/, '/api') : b}/health`)
  ...
  if (loading) {
    return (
      <div style={{
        ...
        `}</style>
        {serverOffline && (
          <div style={{ marginTop: 30, padding: "12px 18px", borderRadius: 12, background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.2)", color: "#f87171", fontSize: 13, maxWidth: 500, textAlign: "center" }}>
            ⚠ Бекенд не отвечает. Запустите: <code style={{ color: "#D8FF47", background: "rgba(216,255,71,.1)", padding: "2px 6px", borderRadius: 4 }}>cd backend && npm start</code>
          </div>
        )}
      </div>
    );
  ...
  return (
    ...
    <div style={{ display: "flex", minHeight: "100vh", width: "100%", overflow: "hidden" }}>
      ...
      <main style={{
        ...
        {serverOffline && (
          <div style={{ marginBottom: 16, padding: "10px 14px", borderRadius: 10, background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.18)", color: "#f87171", fontSize: 12, display: "flex", alignItems: "center", gap: 8 }}>
            <span>⚠</span>
            <span>Бекенд не отвечает. Данные могут не загрузиться. Запустите: <code style={{ color: "#D8FF47", background: "rgba(216,255,71,.08)", padding: "1px 5px", borderRadius: 4 }}>cd backend && npm start</code></span>
          </div>
        )}
        {noResult && (
