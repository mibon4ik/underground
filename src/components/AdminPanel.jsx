...
let raw = import.meta.env.VITE_API_URL || '/api';
if (!raw.startsWith('http') && !raw.startsWith('/')) raw = 'https://' + raw;
...
const hasSession = () => {
  try {
    ...
    return s && token && Date.now() < s.exp;
  } catch { return false; }
};
const openSession = (userData, token) => {
  localStorage.setItem(SESSION_KEY, JSON.stringify({ ...userData, exp: Date.now() + 30 * 24 * 3600_000 }));
...
const COLOR_KEYS = [
  ...
  { key: 'surface', label: 'Поверхность 1' }, { key: 'surface2', label: 'Поверхность 2' }, { key: 'surface3', label: 'Поверхность 3' },
...
function LoginScreen({ onSuccess, onClose }) {
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (data.success) {
        openSession(data.user, data.token);
        onSuccess();
      } else {
...
export default function AdminPanel({ data, onUpdate, onReset, onExport, onImport, onClose, onNavigate }) {
  ...
  const CATEGORIES = [
    {
      ...
      tabs: [
        { id: 'pages', label: '📄 Страницы' }, { id: 'prices', label: '💰 Прайс' },
        { id: 'coaches', label: '🏆 Тренеры' }, { id: 'branches', label: '📍 Филиалы' },
        { id: 'schedule', label: '🕒 Расписание' }, { id: 'programs', label: '💪 Программы' },
        { id: 'shop', label: '🛒 Магазин' }, { id: 'booking', label: '🗓 Бронь' },
        { id: 'promos', label: '🏷 Акции' }, { id: 'achievements', label: '🥇 Успехи' },
      ],
    },
    {
      id: 'content', label: '📝 Контент',
      tabs: [
        { id: 'news', label: '🔥 Новости' }, { id: 'gallery', label: '📸 Галерея' },
        { id: 'reviews', label: '⭐ Отзывы' }, { id: 'faq', label: '❓ FAQ' },
      ],
    },
    {
      id: 'comm', label: '💬 Коммуникация',
      tabs: [
        { id: 'leads', label: '📨 Лиды' }, { id: 'objections', label: '💬 Возражения' },
      ],
    },
    {
      id: 'settings', label: '⚙️ Настройки',
      tabs: [
        { id: 'calc', label: '🧮 % Рассрочки' }, { id: 'referral', label: '🎁 Реферал' },
        { id: 'theme', label: '🎨 Тема' }, { id: 'data', label: '💾 Данные' },
      ],
    },
  ];
  ...
  try {
        ...
        <div className="ap-panel">
          <div className="ap-head">
            <div className="ap-head-l">
              ...
              <span className="ap-user-badge">👤 {getUser()?.login || 'Admin'}</span>
            </div>
          ...
          <div className="ap-tabs" style={{ gap: 2 }}>
            {CATEGORIES.map(c => (
              <button key={c.id} className={`ap-tab${cat === c.id ? ' on' : ''}`} onClick={() => { setCat(c.id); setTab(c.tabs[0]?.id || c.tabs.id); }}>
                {c.label}
              </button>
            ))}
            <span style={{ width: 1, height: 20, background: '#1e2b3a', margin: 'auto 4px', flexShrink: 0 }} />
            {visibleTabs.map(t => (
              <button key={t.id} className={`ap-tab${tab === t.id ? ' on' : ''}`} style={{ fontSize: 10 }} onClick={() => setTab(t.id)}>
                {t.label}
              </button>
            ))}
          </div>
          <div className="ap-body">
            {tab === 'pages' && <PagesTab data={data} save={update} onNavigate={onNavigate} onClose={onClose} />}
            {tab === 'prices' && <ListEditorTab title="Прайс-лист" data={data?.prices || []} save={v => update(['prices'], v)} fields={[{ key: 'name', label: 'Название' }, { key: 'price', label: 'Цена' }]} />}
            {tab === 'coaches' && <ListEditorTab title="Тренеры" data={data?.coaches || []} save={v => update(['coaches'], v)} fields={[{ key: 'name', label: 'ФИО' }, { key: 'regalia', label: 'Регалии' }, { key: 'branch', label: 'Филиал' }]} />}
            {tab === 'branches' && <ListEditorTab title="Филиалы" data={data?.branches || []} save={v => update(['branches'], v)} fields={[{ key: 'name', label: 'Название' }, { key: 'address', label: 'Адрес' }, { key: 'video', label: 'Ссылка на видеообзор' }]} />}
            {tab === 'leads' && <LeadsTab data={data?.leads || []} />}
            {tab === 'schedule' && <ListEditorTab title="Расписание" data={data?.schedule || []} save={v => update(['schedule'], v)} fields={[{ key: 'day', label: 'День' }, { key: 'time', label: 'Время' }, { key: 'activity', label: 'Занятие' }]} />}
            {tab === 'calc' && <CalcTab data={data?.calculator || {}} save={v => update(['calculator'], v)} />}
            {tab === 'news' && <ListEditorTab title="Новости" data={data?.news || []} save={v => update(['news'], v)} fields={[{ key: 'title', label: 'Заголовок' }, { key: 'date', label: 'Дата' }, { key: 'content', label: 'Текст' }]} />}
            {tab === 'gallery' && <ListEditorTab title="Галерея" data={data?.gallery || []} save={v => update(['gallery'], v)} fields={[{ key: 'url', label: 'URL фото' }, { key: 'caption', label: 'Описание' }]} />}
            {tab === 'reviews' && <ListEditorTab title="Отзывы" data={data?.reviews || []} save={v => update(['reviews'], v)} fields={[{ key: 'author', label: 'Автор' }, { key: 'text', label: 'Текст' }, { key: 'rating', label: 'Рейтинг (1-5)' }]} />}
            {tab === 'programs' && <ListEditorTab title="Программы тренировок" data={data?.workout_programs || []} save={v => update(['workout_programs'], v)} fields={[{ key: 'name', label: 'Название' }, { key: 'goal', label: 'Цель' }]} />}
            {tab === 'shop' && <ListEditorTab title="Магазин" data={data?.shop || []} save={v => update(['shop'], v)} fields={[{ key: 'name', label: 'Товар' }, { key: 'price', label: 'Цена' }]} />}
            {tab === 'booking' && <ListEditorTab title="Бронирование" data={data?.booking || []} save={v => update(['booking'], v)} fields={[{ key: 'client', label: 'Клиент' }, { key: 'date', label: 'Дата' }, { key: 'time', label: 'Время' }]} />}
            {tab === 'referral' && <ListEditorTab title="Реферальная система" data={data?.referral || []} save={v => update(['referral'], v)} fields={[{ key: 'policy', label: 'Правило' }, { key: 'bonus', label: 'Бонус' }]} />}
            {tab === 'promos' && <ListEditorTab title="Акции" data={data?.promos || []} save={v => update(['promos'], v)} fields={[{ key: 'title', label: 'Название' }, { key: 'discount', label: 'Скидка' }, { key: 'ends', label: 'До какого числа' }]} />}
            {tab === 'faq' && <ListEditorTab title="FAQ" data={data?.faq || []} save={v => update(['faq'], v)} fields={[{ key: 'q', label: 'Вопрос' }, { key: 'a', label: 'Ответ' }]} />}
            {tab === 'achievements' && <ListEditorTab title="Успехи" data={data?.achievements || []} save={v => update(['achievements'], v)} fields={[{ key: 'title', label: 'Заголовок' }, { key: 'desc', label: 'Описание' }, { key: 'date', label: 'Дата' }]} />}
            {tab === 'objections' && <ObjTab data={data} save={update} expObj={expObj} setExpObj={setExpObj} />}
            {tab === 'theme' && <ThemeTab data={data} save={update} />}
    ...
    return (
      <div className="ap-overlay" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: '#121928', padding: 30, borderRadius: 20, border: '1px solid #ef4444', color: '#eef4fb', maxWidth: 400 }}>
          <h2 style={{ color: '#ef4444' }}>Ошибка панели</h2>
          <p>Произошла критическая ошибка при отрисовке админ-панели.</p>
          <pre style={{ background: '#080c14', padding: 10, borderRadius: 8, fontSize: 12, overflow: 'auto' }}>
            {err.stack || err.message}
          </pre>
          <button className="ap-btn ap-btn-accent" onClick={onClose} style={{ marginTop: 20 }}>Закрыть</button>
        </div>
      </div>
    );
  }
...
function ObjTab({ data, save, expObj, setExpObj }) {
  ...
  const addObj = () => {
    const n = {
      id: `obj_${Date.now()}`, icon: '💬', title: 'Новое возражение', category: 'classic', homeDesc: 'Описание.', principle: 'Принцип.', tip: '',
...
/* ══════════════════════════════════════════════════════════════
   ACCESS TAB
   (Modified to hide password change as it's now backend-only)
══════════════════════════════════════════════════════════════ */
function AccessTab({ logout }) {
  return (<>
    <div className="ap-sec">Сессия</div>
    <div className="ap-info">
      Вы вошли как: <strong style={{ color: '#d8ff47' }}>{getUser()?.login}</strong><br />
      Авторизация выполнена через сервер.
    </div>
...
/* ══════════════════════════════════════════════════════════════
   GENERIC LIST EDITOR
══════════════════════════════════════════════════════════════ */
function ListEditorTab({ title, data = [], save, fields = [] }) {
  const [editingId, setEditingId] = useState(null);

  const addItem = () => {
    const newItem = { id: Date.now(), ...fields.reduce((acc, f) => ({ ...acc, [f.key]: '' }), {}) };
    save([...data, newItem]);
    setEditingId(newItem.id);
  };

  const updateItem = (id, field, value) => {
    save(data.map(item => item.id === id ? { ...item, [field]: value } : item));
  };
  ...
  return (<>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
      <span className="ap-sec" style={{ margin: 0, flex: 1 }}>{title} ({data.length})</span>
      <button className="ap-btn ap-btn-accent" onClick={addItem}>+ Добавить</button>
    </div>
    <div className="ap-block-list">
      {(data || []).map((item, idx) => (
        <div key={item?.id || idx} className="ap-block-row">
          <div className="ap-block-row-head" onClick={() => setEditingId(editingId === item?.id ? null : item?.id)}>
            <span className="ap-block-type-label">
              {item?.[fields[0]?.key] || 'Без названия'}
              {fields[1] && item?.[fields[1]?.key] && <span style={{ color: 'var(--muted)', fontWeight: 400, marginLeft: 6 }}>— {item?.[fields[1]?.key]}</span>}
            </span>
            <button className="ap-li-del" onClick={(e) => { e.stopPropagation(); deleteItem(item?.id); }}>✕</button>
          </div>
          {editingId === item?.id && (
            <div className="ap-block-body">
              {fields.map(f => (
                <div className="ap-f" key={f.key}>
                  <label className="ap-lbl">{f.label}</label>
                  {f.key === 'content' || f.key === 'text' || f.key === 'a' || f.key === 'regalia' ? (
                    <textarea className="ap-ta" value={item?.[f.key] || ''} onChange={e => updateItem(item?.id, f.key, e.target.value)} />
                  ) : (
                    <input className="ap-in" value={item?.[f.key] || ''} onChange={e => updateItem(item?.id, f.key, e.target.value)} />
                  )}
                </div>
              ))}
            </div>
    ...
    {data.length === 0 && <div className="ap-info">Список пуст.</div>}
    <button className="ap-add-row" onClick={addItem}>+ Добавить элемент</button>
  </>);
}
...
function LeadsTab({ data = [] }) {
  return (<>
    <div className="ap-sec">Полученные лиды ({data.length})</div>
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ borderBottom: '1px solid #1e2b3a', textAlign: 'left' }}>
        ...
        <tbody>
          {(data || []).map((l, i) => (
            <tr key={l?.id || i} style={{ borderBottom: '1px solid #0f1520' }}>
              <td style={{ padding: 8, color: 'var(--muted)' }}>{l?.date ? new Date(l.date).toLocaleDateString() : l?.timestamp ? new Date(l.timestamp).toLocaleDateString() : '—'}</td>
              <td style={{ padding: 8, color: '#eef4fb', fontWeight: 700 }}>{l?.name || '—'}</td>
              <td style={{ padding: 8, color: '#d8ff47' }}>{l?.phone || '—'}</td>
              <td style={{ padding: 8, color: 'var(--muted)' }}>{l?.source || 'Форма'}</td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
    {data.length === 0 && <div className="ap-info" style={{ marginTop: 12 }}>Лидов пока нет.</div>}
  </>);
}
...
/* ══════════════════════════════════════════════════════════════
   CALC TAB
══════════════════════════════════════════════════════════════ */
function CalcTab({ data = {}, save }) {
  return (<>
    <div className="ap-sec">Настройки рассрочки</div>
    <div className="ap-f">
      <label className="ap-lbl">% Переплаты (Каспи)</label>
      <input className="ap-in" type="number" value={data.kaspiPercent || 0} onChange={e => save(['calculator', 'kaspiPercent'], Number(e.target.value))} />
    </div>
    <div className="ap-f">
      <label className="ap-lbl">Доступные сроки (мес, через запятую)</label>
      <input className="ap-in" value={(data.terms || []).join(', ')} onChange={e => save(['calculator', 'terms'], e.target.value.split(',').map(v => parseInt(v.trim())).filter(v => !isNaN(v)))} />
    </div>
  </>);
}
