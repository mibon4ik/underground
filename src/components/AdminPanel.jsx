...
const openSession = (userData) => localStorage.setItem(SESSION_KEY, JSON.stringify({ ...userData, exp: Date.now() + 30 * 24 * 3600_000 }));
        ...
        <tbody>
          {(data || []).map((l, i) => (
            <tr key={l?.id || i} style={{ borderBottom: '1px solid #0f1520' }}>
              <td style={{ padding: 8, color: 'var(--muted)' }}>{l?.timestamp ? new Date(l.timestamp).toLocaleDateString() : '—'}</td>
