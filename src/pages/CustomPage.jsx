...
function TextBlock({ block }) {
  return (
    <div className="card">
      ...
      <div style={{ color: 'var(--text)', opacity: .9, lineHeight: 1.8, fontSize: 15, whiteSpace: 'pre-wrap' }}>{block.content}</div>
    </div>
  );
}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 14 }}>
        {items.map((item, i) => (
          <div key={i} className="card" style={{ padding: 18 }}>
            ...
            {item.title && <h4 style={{ fontSize: 15, fontWeight: 800, marginBottom: 8, color: 'var(--text)' }}>{item.title}</h4>}
        ...
        {items.map((item, i) => (
          <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, paddingTop: 3 }}>
              {typeof item === 'string'
                ? <span style={{ color: 'var(--text)', opacity: .85, lineHeight: 1.7 }}>{item}</span>
                : <>
                    {item.title && <strong style={{ display: 'block', color: 'var(--text)', marginBottom: 3 }}>{item.title}</strong>}
            ...
            {rows.map((row, ri) => (
              <tr key={ri} style={{ borderBottom: ri < rows.length - 1 ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                {(Array.isArray(row) ? row : [row]).map((cell, ci) => (
                  <td key={ci} style={{ padding: '11px 16px', color: ci === 0 ? 'var(--text)' : 'var(--muted)', lineHeight: 1.6 }}>{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
...
function AlertBlock({ block }) {
  ...
  return (
    <div style={{ padding: '16px 18px', borderRadius: 16, background: s.bg, border: `1px solid ${s.border}`, lineHeight: 1.7 }}>
      ...
      <span style={{ color: 'var(--text)', opacity: .85, fontSize: 14 }}>{block.text}</span>
    </div>
  );
}
